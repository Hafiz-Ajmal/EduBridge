...
from models import StudentCreate,Student,UserDB,StudentOut,StudentUpdate,StudentRegister,Section
...
def add_student(data:StudentRegister,session:session_Dep,current_user:Annotated[UserDB,Depends(require_roles(["admin", "super_admin"]))]):
    ...
    user.is_active=True
    session.add(user)
    #session.commit() no two time commit just one time is better
    session.flush() 
