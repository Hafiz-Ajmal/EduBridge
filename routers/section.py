...
from models import Section,SectionCreate,SectionOut,SectionUpdate,UserDB,Class,TeacherSection
...
def add_new_section(section:SectionCreate,session:session_Dep,current_user:Annotated[UserDB,Depends(require_roles(["admin", "super_admin"]))]):
    ...
    if section_deuplicate:
        raise HTTPException(status_code=402,detail="Section Already Exist For this Class")
...
def delete_section(section_id:int,session:session_Dep,current_user:Annotated[UserDB,Depends(require_roles(["admin","super_admin"]))]):
    ...
    teacher_secs=session.exec(select(TeacherSection).where(TeacherSection.section_id==section_id)).all()
    
    if teacher_secs:

        for teacher_sec in teacher_secs:
...
def get_section_complete(id:int,session:session_Dep,current_user:Annotated[UserDB,Depends(require_roles(["admin","super_admin"]))]):
    ...
    return {"section":section,
            "Class":section.class_,
            "Teacher IDS For Sections":[teacher.user_id for teacher in section.teachers],
            }
